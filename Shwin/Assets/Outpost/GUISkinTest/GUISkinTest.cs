...
public class GUISkinTest : MonoBehaviour
{
    ...
    void Awake()
    {
		...
		rctWindow1 = new Rect(20, 20, 230, 100);
		rctWindow2 = new Rect(340, 20, 230, 380);
    }
    ...
    void OnGUI()
    {
		if (error_GUISkins == false) {
			...
			rctWindow1 = GUILayout.Window(0, rctWindow1, DoConfigWindow, "GUI Skin Config", GUI.skin.GetStyle("window"));
    ...
    void DoConfigWindow(int windowID)
    {
		...
		for (int t = 0; t < thisGUISkins.Length; t++) {
			...
			if (GUILayout.Button(button_value)) {
				...
				rctWindow1 = new Rect(rctWindow1.x, rctWindow1.y, 230, 100);
				rctWindow2 = new Rect(rctWindow2.x, rctWindow2.y, 230, 380);
			}
    ...
    void DoMyWindow(int windowID)
    {
		...
		GUILayout.BeginHorizontal();
		GUILayout.Space(6);
		scrollPosition = GUILayout.BeginScrollView(scrollPosition, true, true, GUILayout.Height(80));
        for (int i = 0; i < 12; i++)
        {
            GUILayout.Label("Im the #" + (i + 1) +" very long line of text", GUILayout.Width(350));
        }
